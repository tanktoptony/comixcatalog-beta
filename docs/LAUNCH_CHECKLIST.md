...
- [ ] **Priority cover coverage >= 90%** (launch-priority universe: user collections, wantlists, featured titles, frequently searched series, current releases, high-value issues — NOT the full raw catalog)
  - Owner:
  - Evidence: **17,436/25,415 issues covered (68.61%) across 169 series**, measured via `scripts/generatePriorityCoverTargets.js` after fixing a cross-volume cover-bleed bug (`src/app/api/issues/[id]/route.js` — a cover with no `series_year` could be matched to any same-titled issue regardless of actual volume; concrete case: Nova (1994) #4 was showing 2013 Marvel NOW! Nova art). The number is down from the previously-recorded 71.61% — some of that figure was inflated by exactly this class of wrong-volume false match; part of the drop may also be the priority universe itself shifting (169 vs 168 series, 25,415 vs 25,391 issues, live `user_collections`/valuation data changes between runs). Both effects are plausible; not disentangled. This number should be more trustworthy than the prior one, not just lower. 101 series still need work — biggest raw gaps: The Spectacular Spider-Man (1976) 526/649, Marvel Tales (1966) 485/612, G.I. Joe: A Real American Hero (1982) 0/431.
  - Last checked: 2026-08-04 (priority-scoped live query, post cover-bleed fix)
  - Blocker: measured coverage (68.61%) is 21.4 points below the 90% gate. 744 gap-priority targets need manual `--volume-id` resolution (ambiguous title/publisher match) — see `needs_volume_id.json`. Also unresolved: no live current-release or search-frequency signal.
