...
// initialRemaining comes from the server-rendered page (fetched fresh per
// request, see page.js) — this used to default to a hardcoded 83 in
// useState, which flashed to the real count as soon as the client fetch
// below resolved. Same bug and fix as FoundingBanner.js. null means "don't
// know yet"; render a dash rather than a guessed digit.
export default function FoundingCollectorsClient({ initialRemaining = null }) {
  const { user } = useAuth();
  ...
  return (
    <main className="founding-page">
      <section className="founding-hero">
        ...
        <div className="founding-number">{status.remaining == null ? "—" : status.remaining}</div>
